...
public class interfaz_tabla_multiplicar extends JFrame  {
     ...
     interfaz_tabla_multiplicar() {
        ...
        //Clase para que el boton calc este a la escucha
class OyenteMultiplicacion implements ActionListener{
               @Override
               public void actionPerformed(ActionEvent e) {
                    int numSelec = Integer.parseInt(listanumeros.getSelectedItem().toString());
                    for(int x = 0; x <= BotonsNumero1.length - 1;x++){
                         int numBoton = Integer.parseInt(BotonsNumero1[x].getText().toString());
                         int total = numSelec * numBoton;
                         BotonsNumero2[x].setText(String.valueOf(total));
                    }     
          }     
     }

     OyenteMultiplicacion oyenteMultiplicacion = new OyenteMultiplicacion();
     listanumeros.addActionListener(oyenteMultiplicacion);
          class OyenteSuma implements ActionListener {
               ...
               public void actionPerformed(ActionEvent e) {
                      try{
                         int total;
                         total = Integer.parseInt(numero1.getText()) + Integer.parseInt(numero2.getText());
          ...
          OyenteSuma oyenteSuma = new OyenteSuma();
          calc.addActionListener(oyenteSuma);       
     }
